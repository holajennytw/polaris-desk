...
- **不直接推 `main`**：開分支 → push → 開 PR。**不要自己按 Merge** —— 由 code owner（**R2**）審核批准、且 **CI 測試綠燈**後才放行（`main` 已設分支保護 + CODEOWNERS + 必過 CI）。
  ...
  ```
- 🚦 **GitHub API / 留言節流（必守）**：**用 `scripts/gh-throttle.sh` 取代裸 `gh`**，尤其是會「產生內容」的呼叫（`comment` / `create` / `review` / `api -X POST…`）。它會：① 串行（不併發）② 呼叫之間強制最小間隔（mutating 預設 5s，跨多次呼叫也算，避免爆量）③ 遇到 secondary rate limit 自動退避重試 ④ 記錄到 log（`scripts/gh-throttle.sh --audit` 可查）。
  - **留言批次化**：一個 PR / issue 只發 **一則彙整留言**，不要每個發現各發一則。wrapper 設有硬上限（`GH_THROTTLE_COMMENT_CAP`，預設 8 則 / 小時），**超過會直接拒絕（exit 3）** 並要你彙整；編輯既有留言（`--edit-last`）不受限；真的要破例才設 `GH_THROTTLE_FORCE=1`。
  - **不要平行 / 併發**跑多個 `gh`（包含 subagent 同時呼叫）。
  - **背景**：2026-06-22 因短時間 API 呼叫 + 留言爆量，被 GitHub 反濫用機制判定違反 ToS、**暫停帳號**（已走 Account restrictions 申訴）。節流是為了復權後不再觸發。
