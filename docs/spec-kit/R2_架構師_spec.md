...
- [x] D17 G3 評審（架構面）— `docs/G3_readiness.md` 自評：G3 四條件中 R2 直接負責的 **Deep Research 可跑（D15 v0 + D16 v1 過驗收，場景 2 四門檻 ≤6/≥3/可溯源/0 建議全過且可重現，`test_deep_research_acceptance.py` 背書）** 已就緒；**LLMLingua 半邊**＝量測 harness 就緒 + 基線實量 ~7–14%、**≥50% 實測待本機 `[llmlingua]` extra**（D8 刻意「量測 only、未接 live graph」）。W3 交付 D11/D13/D15/D16 全綠、核心不變量持續綠（304 passed, ruff clean）。**跨角色硬門檻**（Eval ≥80%＝R5、ColPali＝R6〔失敗→砍場景3〕、Watchdog＝R3）+ 4 條件逐項 owner 簽核＝R1 彙整（R1 spec D15–16）；本文不越界。判定：**Go（架構面）**
