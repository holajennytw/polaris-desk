...
| 欄位 | 來源 | 說明 |
|------|------|------|
| `company_a/b` | JOIN `company_dim` | R3 帶入中文名，前端不用另查 |
| `kpis[]` | R3 計算自 `financial_metrics` | 前端 KPI Card 用；`a_value`/`b_value` 已格式化為字串（含單位） |
| `kpis[].diff` | R3 計算 | 差異描述，如 `"+19.5pp"`；`better` 為 `"a"` 或 `"b"` |
