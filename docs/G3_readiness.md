...
| 視覺檢索第 4 路（接既有 colpali_pages）| **R3/R4** | **TD-02（PM 簽核 2026-06-23）**：逆轉 TD-01 檢索整合決定。查證後事實——R4 早已將圖表頁以 ColPali 入庫（`polaris_core.colpali_pages`，5701 頁、20 tickers、128 維池化向量），僅 R3 未接。本案接上既有表為**第 4 路（gated，場景 3 專用）**，`origin="colpali"`，**零資料架構變更**（只讀既有表，不動 chunks 768 庫）。Phase 1 檢索+命中率（≥70%），Phase 2 vision 讀數字回答。依賴 R4 提供 query encoder。設計見 `docs/superpowers/specs/2026-06-20-restore-4th-retrieval-path-vision-design.md` | ✅ PM 簽核（WayneSHC，2026-06-23）——僅核可「復原為 gated 第 4 路」。**prod 啟用 3 閘**：① #133 encoder（✅ 併入 jenny/main 226c814）② PM 簽核（✅ 本列）③ ≥70% round-trip（⏳ 待 GPU + R4 gold，見 `scripts/colpali_roundtrip_check.py`）。三閘到齊才設 `COLPALI_QUERY_ENCODER=1` 上 prod |
