...
.PHONY: setup install dev db-up db-down test fmt lint check check-keys bq-smoke audit daily-status daily-status-dry serve serve-api docker-build docker-run eval-smoke
...
eval-smoke:     ## Eval pipeline 煙測（前 5 題、0 token；紅線買賣建議 → exit 1）
	.venv/bin/python -m polaris.eval --mode smoke --quick 5
