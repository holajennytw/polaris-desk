...
const TAB_LABELS: Record<string, string> = { feed:"風險動態", tracking:"追蹤通知", rules:"規則管理" };
    ...
    <div className="page-scroll">
      <div className="page">
        <div className="page-head">
          ...
          <p className="page-desc">風險監控、追蹤動態與規則管理。</p>
        </div>
        ...
        {tab==="feed" && (
          <div className="panel" style={{marginTop:16}}>
            ...
            <div className="alert-list">
              ...
              {allAlerts.length===0 && <div style={{padding:"32px 16px",textAlign:"center",color:"rgb(var(--muted))"}}>目前無風險警示</div>}
            </div>
          </div>
        )}
        {tab==="tracking" && (
          <div className="panel" style={{marginTop:16}}>
            ...
            <div style={{padding:"12px 0"}}>
              {trackItems.length===0 ? (
                <div style={{padding:"32px 16px",textAlign:"center",color:"rgb(var(--muted))"}}>
                  <Icon name="bellOff" size={32} style={{marginBottom:10,opacity:0.4}}/>
                  <div>目前無追蹤通知</div>
                </div>
              ) : trackItems.map(n=>(
        ...
        {tab==="rules" && (
          <div className="panel" style={{marginTop:16}}>
            <div className="panel-head">
              <span className="panel-title"><Icon name="target" size={15} style={{color:"rgb(var(--primary))",verticalAlign:"-2px",marginRight:6}}/>監控規則</span>
              <button className="btn sm" style={{marginLeft:"auto"}}><Icon name="bolt" size={14}/>新增規則</button>
            </div>
            <div style={{padding:"12px 16px",color:"rgb(var(--muted))",fontSize:15}}>規則管理功能開發中，敬請期待。</div>
          </div>
        )}
      </div>
    </div>
